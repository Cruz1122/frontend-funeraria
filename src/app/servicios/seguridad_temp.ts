...
export class SeguridadService {
  ...
  private datosUsuarioValidado =
    new BehaviorSubject<UsuarioValidadoModel | null>(null);
  ...
  IndentificarUsuario(
    ...
    return this.http.post<UsuarioModel>(
      `${this.urlSeguridad}identificar-usuario`,
      {
        correo: usuario,
        clave: clave,
      }
  ...
  ValidarCodigo2FA(
    ...
    return this.http.post<UsuarioValidadoModel>(
      `${this.urlSeguridad}verificar-2fa`,
      {
        usuarioId: idUsuario,
        codigo2fa: codigo,
      }
  ...
  ObtenerNombreUsuario(): string {
    ...
    if (datos) {
      ...
      let nombre =
        usuario.user?.primerNombre + ' ' + usuario.user?.primerApellido;
      return nombre;
    } else {
      return '';
    }
  ...
  RegistrarUsuarioPublico(datos: any): Observable<UsuarioModel> {
    return this.http.post<UsuarioModel>(
      `${this.urlSeguridad}usuario-publico`,
      datos
    );
  }
  ...
  RegistrarUsuarioPublicoLogicaNegocio(datos: any): Observable<UsuarioModel> {
    return this.http.post<UsuarioModel>(
      `${this.urlLogicaNegocio}cliente-publico`,
      datos
    );
  }
  ...
  EnviarPQRS(datos: any): Observable<PqrsModel> {
    return this.http.post<PqrsModel>(
      `${this.urlLogicaNegocio}generar-pqrs`,
      datos
    );
  }
  ...
  CambiarClave(datos: any): Observable<UsuarioModel> {
    return this.http.post<UsuarioModel>(
      `${this.urlSeguridad}cambio-clave`,
      datos
    );
  }
  ...
  ObtenerDatosUsuarioCliente(idUsuario: string): Observable<ClienteModel> {
    ...
    return this.http.get<ClienteModel>(
      `${this.urlLogicaNegocio}cliente-por-idseguridad/${idUsuario}`
    );
  }
  ...
  VerificarSalaChat(codigo: string): Observable<{ exists: boolean }> {
    ...
    return this.http.get<{ exists: boolean }>(
      `${this.baseUrl}/check-chat-room/${codigo}`
    );
  }
}
